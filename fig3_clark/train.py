...
if __name__ == '__main__':
    ...
    ap.add_argument("--output", type=str, default='Butyrate')
    ap.add_argument("--test_train_frac", type=float, default=0.8)
    ...
    len_data = len(ClarkDataset(batch_size=1, num_workers=0, device='cuda:0'))
    print("Len data", len_data)

    rng = np.random.default_rng(args.seed)
    train_idx = [rng.choice(len_data, size=int(len_data * args.test_train_frac), replace=False) for _ in range(args.N_ensemble)]
    test_idx = [np.asarray([x for x in range(len_data) if x not in train_idx[i]]) for i in range(args.N_ensemble)]
    ...
    dataset_train = ClarkDataset(**train_dataset_kwargs)
    dataset_test = ClarkDataset(**test_dataset_kwargs)
        ...
        for ens_idx in range(ensemble_size):
            ...
            for sample, sample_test in zip(dataset_train.loader_list[ens_idx], dataset_test.loader_list[ens_idx]):
                ...
                for m, nc_idx in enumerate(models):
                    ...
                    if model.isnan or model.early_stop:
                        continue

                    model.optimizer.zero_grad()
                    proj_inpt = sample['input'] @ model.proj_mat()
                    pred = model(proj_inpt, None).squeeze()
                    ...
                    if count % args.save_freq == 0:
                        ...
                        if len(train_losses[nc_idx][ens_idx]) > early_stopping_limit:
                            if np.all(test_losses[nc_idx][ens_idx][-early_stopping_limit] < test_losses[nc_idx][ens_idx][-early_stopping_limit + 1:]):
                                model.early_stop = True
                                print(f"Early stopping model {nc_idx} ensemble {ens_idx} at step {count}")
                        ...
                        torch.save({
                            ...
                            'probs': model.deterministic_proj_mat().cpu().detach().numpy(),
