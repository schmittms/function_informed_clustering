...
class TaraDataset(Dataset):
    def __init__(self,
                 batch_size=16,
                 num_workers=0,
                 device='cpu',
                 train_idx=None,
                 targets=['Nitrates', 'Oxygen'],
        ...
        self.df = pd.read_csv(source_file_path)
        ...
        abd_cols = self.df.columns.str.contains('Bacteria') | self.df.columns.str.contains('Archaea')
        self.col_names = self.df.columns[abd_cols]
