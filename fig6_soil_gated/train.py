...
if __name__ == '__main__':
    ...
    ap.add_argument("--targets", type=str, default='no3')
    ap.add_argument("--normalize_target", type=int, default=0)
    ...
    nsample = 254 if args.CHL == 'both' else 127
    rng = np.random.default_rng(args.seed)
    train_idx = [rng.choice(nsample, size=int(nsample * 0.75), replace=False) for _ in range(args.N_ensemble)]
    ...
    train_dataset_kwargs = {
        ...
        'pred_vars': args.targets,
        'train_idx': train_idx,
        'normalize_target': int(args.normalize_target),
    }
    test_dataset_kwargs = {
    ...
    for t in TAXA:
        ...
        for n_clust in [1, 2, 3, 4, 6]:
            ...
            model_kwargs_all[t][n_clust] = model_kwargs
            for idx in range(ensemble_size):
                model = GatedModel(**model_kwargs, freeze_proj=False)
                model.to(device)
                model.device = device
                model.train()
                models[t][n_clust].append(model)

    # Training loop
    count = 0
    t0 = time.time()
    train_losses = {t: {nc: [[] for _ in range(len(models[t][nc]))] for nc in models[t]} for t in models}
    gate_losses = {t: {nc: [[] for _ in range(len(models[t][nc]))] for nc in models[t]} for t in models}
    test_losses = {t: {nc: [[] for _ in range(len(models[t][nc]))] for nc in models[t]} for t in models}
    ...
    while count < args.n_steps:
        for ens_idx in range(ensemble_size):
            ...
            for sample, sample_test in zip(dataset_train.loader_list[ens_idx], dataset_test.loader_list[ens_idx]):
                ...
                for t, tx in enumerate(TAXA):
                    for m, nc_idx in enumerate(models[tx]):
                        model = models[tx][nc_idx][ens_idx]
                        if model.isnan:
                            continue

                        model.optimizer.zero_grad()
                        proj_inpt = (sample['input'][tx] * model.gate().T) @ model.proj_mat()
                        pred = model(proj_inpt, None)
                        assert pred.shape == sample['target'].shape, f"Prediction shape {pred.shape} does not match target shape {sample['target'].shape}"
                        ...
                        with torch.no_grad():
                            test_proj_inpt = (sample_test['input'][tx] * model.gate().T) @ model.proj_mat()
                        ...
                        if np.isnan(loss.detach().cpu().numpy()):
                            model.isnan = True
                            train_losses[tx][nc_idx][ens_idx].append(np.nan)
                            test_losses[tx][nc_idx][ens_idx].append(np.nan)
                            gate_losses[tx][nc_idx][ens_idx].append(np.nan)

                        if count % args.save_freq == 0:
                            train_losses[tx][nc_idx][ens_idx].append(reconstr_loss.detach().cpu().numpy())
                            test_losses[tx][nc_idx][ens_idx].append(test_loss.detach().cpu().numpy())
                            gate_losses[tx][nc_idx][ens_idx].append(gate_loss.detach().cpu().numpy())
                            ...
                            torch.save({
                                ...
                                'curr_tau': model.tau,
                                'tau_hparams': model_kwargs_all[tx][nc_idx]['tau_hparams'],
                                'ens_idx': ens_idx,
                                'taxon': tx,
                                'seed': args.seed,
                                'train_idx_split': np.asarray(train_idx[ens_idx]),
                                'test_idx_split': np.asarray(test_idx[ens_idx]),
                                'train_losses': train_losses[tx][nc_idx][ens_idx],
                                'test_losses': test_losses[tx][nc_idx][ens_idx],
                                'gate_losses': gate_losses[tx][nc_idx][ens_idx],
                                'train_dataset_kwargs': train_dataset_kwargs,
                                'test_dataset_kwargs': test_dataset_kwargs,
                                'model_kwargs': model_kwargs_all[tx],
                            }, os.path.join(args.save_dir, f'tax-{tx}_Npcs-{nc_idx}_model-{ens_idx}.pt'))
        ...
        if count % args.save_freq == 0:
            ...
            output_str = f'\tLoss:\t{train_losses[TAXA[0]][n_pc][-1][-1]:0.4f}' + \
                         ...
                         f'\tGate:\t{gate_losses[TAXA[0]][n_pc][-1][-1]:0.4f}'
